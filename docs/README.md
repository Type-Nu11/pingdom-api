...
- **관리자 운영**: 사용자 제재, 신고 승인/반려, 게시글 삭제, 장소 병합, 추천 지표 조회를 제공합니다.
...
| Admin API | 신고, 게시글, 사용자 제재, 장소, 추천 지표 관리 API |
...
- [커뮤니티 조회 및 삭제 데이터 노출 정책](community-query-deleted-data-policy.md)
...
- **Moderation**: 신고 승인/반려, 사용자 제재/해제, 제재 이력 조회, 관리자 게시글/장소 관리
...
운영 `compose.yaml`의 app 포트는 기본적으로 `127.0.0.1:8080`에 바인딩합니다. 별도 서버의 프록시를 사용할 때는 실제 연결 경로와 출발지 제한을 확인한 후 `PINGDOM_APP_BIND_ADDRESS`를 지정합니다. 배포 readiness probe는 실제 Compose 바인딩을 사용합니다. 설정과 복구 절차는 [프록시 연결 및 Swagger 운영 안내](operations/proxy-swagger-access.md)를 따릅니다.
...
├── moderation      # 관리자 조회, 신고 처리, 사용자 제재
