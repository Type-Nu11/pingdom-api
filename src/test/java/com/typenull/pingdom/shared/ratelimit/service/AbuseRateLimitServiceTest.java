...
class AbuseRateLimitServiceTest {
    ...
    /**
     * 고정 Clock과 메모리 저장소에 사용자별 2회 제한·재전송 1분 간격 등을 설정해 정책 키와 시간 경계를 검증한다.
     */
    @BeforeEach
    void setUp() {
    ...
    /**
     * 대소문자·공백만 다른 이메일은 IP가 달라도 같은 가입 횟수 창을 공유하여 세 번째 요청이 거절되는지 검증한다.
     */
    @Test
    ...
    /**
     * 사용자명 대소문자를 정규화해 세 번째 로그인을 거절하고 1분 경계 뒤 다시 허용하는지 검증한다.
     */
    @Test
    ...
    /**
     * 같은 IP의 세 번째 상담 도입 요청은 거절하고 다른 IP는 허용해 익명 요청 제한 범위를 검증한다.
     */
    @Test
    ...
    /**
     * 대소문자가 다른 같은 이메일도 재전송 쿨다운을 공유하며 1분 경계 뒤 재전송이 허용되는지 검증한다.
     */
    @Test
    ...
    /**
     * 대소문자·공백만 다른 이메일 인증 요청이 IP와 무관하게 같은 횟수 제한을 공유하는지 검증한다.
     */
    @Test
    ...
    /**
     * 비밀번호 초기화 요청이 이메일 정규화 후 1분 재요청 간격을 적용하고 경계 시각에는 허용되는지 검증한다.
     */
    @Test
    ...
    /**
     * 대소문자만 다른 초기화 토큰은 서로 다른 제한 키를 만들어 토큰 식별자를 소문자로 합치지 않는지 검증한다.
     */
    @Test
    ...
    /**
     * 사용자 1의 두 번째 업로드는 다른 IP라도 거절하고 사용자 2는 허용해 사용자별 창을 검증한다.
     */
    @Test
    ...
    /**
     * 사용자 1의 세 번째 추천 클릭은 IP가 바뀌어도 거절하고 다른 사용자는 허용하는지 검증한다.
     */
    @Test
    ...
    /**
     * 대소문자만 다른 리프레시 토큰은 서로 다른 제한 키를 사용해 토큰 식별을 보존하는지 검증한다.
     */
    @Test
    ...
    private static class MutableClock extends Clock {
        ...
        /**
         * 제한 창과 쿨다운 경계를 재현할 초기 Instant·시간대를 보관한다.
         */
        ...
        /**
         * 동일 현재 시각에 새 시간대를 가진 독립 Clock을 제공한다.
         */
        @Override
        ...
        /**
         * 테스트가 전진시킨 현재 Instant를 제한 저장소에 제공한다.
         */
        @Override
        ...
        /**
         * 실제 대기 없이 시간을 전진시켜 고정 창 만료와 쿨다운 해제를 재현한다.
         */
    ...
    private static class FakeRateLimitStore implements RateLimitStore {
        ...
        /**
         * 시간 제어용 Clock을 저장해 메모리 창·쿨다운의 만료를 판정한다.
         */
        ...
        /**
         * 쿨다운과 모든 창의 한도를 먼저 확인한 뒤 허용 요청만 카운터·다음 허용 시각에 반영한다.
         * 정책 단위 테스트용 순차 대역이며 Redis 원자성이나 실제 동시 경합을 검증하지 않는다.
         */
        @Override
        public void acquire(
                String message,
        ...
        /**
         * 창이 없거나 만료 경계에 도달하면 새 0회 창을 만들고 아직 유효한 창은 유지한다.
         */
        ...
        private static final class WindowState {
            ...
            /**
             * 메모리 제한 창의 현재 요청 횟수와 만료 시각을 보관한다.
             */
        ...
        /**
         * 마지막 acquire에 전달된 창 키를 노출해 토큰 fingerprint의 대소문자 보존을 비교한다.
         */
