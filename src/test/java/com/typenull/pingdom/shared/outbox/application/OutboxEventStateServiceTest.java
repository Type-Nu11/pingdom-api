...
class OutboxEventStateServiceTest {
    ...
    /**
     * 최대 5회·기본 10초 backoff와 고정 Clock으로 Outbox 실패·수동 복구 서비스를 구성.
     */
    @BeforeEach
    void setUp() {
    ...
    /**
     * 연속 실패의 시도 수가 1·2로 증가하고 고정 현재 시각 기준 다음 시도가 각각 10초·20초 뒤로 설정되는지 검증.
     */
    @Test
    ...
    /**
     * 5회 실패한 이벤트를 잠금 조회해 수동 재시도하면 이전 FAILED·이후 RETRY를 반환하고 시도 수와 오류를 초기화하는지 검증.
     */
    @Test
    ...
    /**
     * PENDING 이벤트의 수동 재시도는 NOT_RETRYABLE이며 상태가 바뀌지 않는지 검증.
     */
    @Test
    ...
    /**
     * 1분 전 생성하고 1초 전 선점한 좋아요 이벤트를 만들어 실패 처리가 가능한 상태를 제공.
     */
