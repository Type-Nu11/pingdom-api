...
class GlobalExceptionHandlerTest {
    ...
    /**
     * 인증 메트릭 대역을 주입한 공통 예외 처리기를 만들어 응답 매핑과 관측 호출을 분리 검증한다.
     */
    @BeforeEach
    void setUp() {
    ...
    /**
     * INVALID_TOKEN 도메인 예외의 상태·ErrorResponse와 controller_advice 출처 인증 실패 메트릭을 확인한다.
     */
    @Test
    ...
    /**
     * 요청 본문의 name 필드 검증 오류를 VALIDATION_FAILED 코드와 필드별 메시지로 매핑하는지 검증한다.
     */
    @Test
    ...
    /**
     * ConstraintViolation의 request.id 경로·메시지가 공통 검증 오류 응답에 보존되는지 검증한다.
     */
    @Test
    ...
    /**
     * 예상하지 못한 예외는 INTERNAL_SERVER_ERROR로 응답하고 내부 secret 문자열을 메시지에 노출하지 않는지 검증한다.
     */
    @Test
