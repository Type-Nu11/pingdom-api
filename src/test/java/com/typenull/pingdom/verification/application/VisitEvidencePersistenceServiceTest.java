...
class VisitEvidencePersistenceServiceTest {
    ...
    /** 사용자 1이 체크인 2를 소유한 것으로 조회 mock을 구성해 저장 제약 처리에 집중. */
    @BeforeEach
    void setUp() {
    ...
    /**
     * 체크인 증빙 유일 제약 위반을 중첩 Hibernate 예외로 구성.
     * 저장 시 VISIT_EVIDENCE_ALREADY_EXISTS로 변환되어야 함.
     */
    @Test
    ...
    /**
     * 외래 키 제약 위반은 중복 증빙 오류로 오인하지 않아야 함.
     * 처음 발생한 DataIntegrityViolationException 인스턴스가 그대로 전달되는지 확인.
     */
    @Test
