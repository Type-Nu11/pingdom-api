...
class VisitEvidenceFileValidatorTest {
    ...
    /**
     * 실제 PNG 바이트와 선언한 image/png가 일치하면 검증을 통과해야 한다.
     * 반환된 MIME 타입과 원본 파일명이 유지되는지 확인한다.
     */
    @Test
    ...
    /** JPEG 바이트를 PNG라고 선언하면 파일 확장자와 무관하게 잘못된 증빙 오류로 거부한다. */
    @Test
    ...
    /**
     * 최대 10바이트 설정에 11바이트를 전달하면 크기 초과 오류가 발생해야 한다.
     * 이미지 판독 전 요청 크기를 제한하는 경계를 확인한다.
     */
    @Test
    ...
    /**
     * PNG 시그니처만 있고 유효한 이미지 본문이 없는 입력을 거부한다.
     * 매직 바이트가 맞다는 이유만으로 손상된 이미지를 허용하는 회귀를 방지한다.
     */
    @Test
    ...
    /**
     * 정상 PNG의 너비 헤더를 8,001로 바꾼 입력이 파일 형식 오류로 거부되는지 확인한다.
     * 너비 제한 8,000을 넘는 입력을 대상으로 하며 디코더 내부 호출 순서를 별도로 검증하지는 않는다.
     */
    @Test
    ...
    /** 지정한 포맷으로 2×2 RGB 이미지를 메모리에서 인코딩해 정상 이미지 바이트를 제공한다. */
    ...
    /** 파일 검증이 방문 인증 예외로 실패하는지와 그 오류 코드가 기대값인지 함께 확인한다. */
