...
class AdminPictureControllerTest {
    ...
    @Test
    void listPicturesIncludesReportsInSamePictureItem() throws Exception {
        String adminAccessToken = createAdminAndLogin();
        User owner = createUser("pictureOwner01");
        MapImage mapImage = createMapImage(owner.getId(), owner.getUsername(), "https://example.com/image-1.jpg");
        PictureReport olderReport = createPictureReport(11L, "reporter01", mapImage, "첫 번째 신고");
        PictureReport newerReport = createPictureReport(12L, "reporter02", mapImage, "두 번째 신고");
        ...
        mockMvc.perform(get("/admin/pictures")
                ...
                .andExpect(jsonPath("$.pictures[0].id").value(mapImage.getId()))
                .andExpect(jsonPath("$.pictures[0].reports.length()").value(2))
                .andExpect(jsonPath("$.pictures[0].reports[0].reportId").value(newerReport.getId()))
                .andExpect(jsonPath("$.pictures[0].reports[0].reporterUserId").value(12L))
                .andExpect(jsonPath("$.pictures[0].reports[0].reporterUsername").value("reporter02"))
                .andExpect(jsonPath("$.pictures[0].reports[0].reason").value("두 번째 신고"))
                .andExpect(jsonPath("$.pictures[0].reports[0].status").value(PictureReportStatus.PENDING.name()))
                .andExpect(jsonPath("$.pictures[0].reports[0].processedAt").doesNotExist())
                .andExpect(jsonPath("$.pictures[0].reports[1].reportId").value(olderReport.getId()));
    }

    @Test
    void listPicturesReturnsEmptyReportsWhenNoReportExists() throws Exception {
        String adminAccessToken = createAdminAndLogin();
        User owner = createUser("pictureOwner02");
        ...
        mockMvc.perform(get("/admin/pictures")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.pictures[0].id").value(mapImage.getId()))
                .andExpect(jsonPath("$.pictures[0].reports.length()").value(0));
    }
