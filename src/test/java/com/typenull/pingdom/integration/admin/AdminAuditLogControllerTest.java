...
/**
 * 관리자 세부 역할과 감사 로그 필터·페이지 계약을 실제 로그인 및 저장 데이터로 검증.
 */
@Tag("integration")
@SpringBootTest
...
class AdminAuditLogControllerTest {
    ...
    /**
     * 감사 로그와 관리자 역할을 사용자보다 먼저 비워 조회 결과를 격리.
     */
    @BeforeEach
    void setUp() {
    ...
    /**
     * ANALYST는 감사 로그를 조회하지만 SUPPORT_OPERATOR는 ADMIN_PERMISSION_REQUIRED로 거절되는지 확인.
     */
    @Test
    ...
    /**
     * 행위·대상 종류·대상 ID·기간을 함께 적용한 결과가 해당 감사 로그 한 건인지 확인.
     */
    @Test
    ...
    /**
     * 선택 필터를 생략한 조회와 전체 삭제 후 빈 페이지의 건수·페이지 수·다음 페이지 여부를 확인.
     */
    @Test
    ...
    /**
     * 시작이 종료보다 늦은 기간이 INVALID_AUDIT_LOG_FILTER_PERIOD로 거절되는지 확인.
     */
    @Test
    ...
    /**
     * 일반 사용자 토큰으로 감사 로그 조회 시 403과 ACCESS_DENIED를 확인.
     */
    @Test
    ...
    /**
     * 지정 역할의 사용자를 저장하며 ADMIN이면 SUPER_ADMIN을 배정한 뒤 실제 로그인 토큰을 반환.
     */
    ...
    /**
     * 지정한 세부 관리자 역할을 배정해 권한 경계를 검증할 로그인 토큰을 생성.
     */
    ...
    /**
     * 공통 비밀번호로 로그인 성공을 확인하고 JSON 접근 토큰을 추출.
     */
