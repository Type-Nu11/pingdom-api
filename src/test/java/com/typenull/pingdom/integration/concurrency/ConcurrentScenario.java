...
/**
 * 두 작업의 출발을 맞추고 성공값과 예외를 모아 DB 동시성 테스트가 승자·패자를 비교하도록 돕는다.
 */
final class ConcurrentScenario {

    /**
     * barrier와 executor를 정적 run 호출마다 생성하므로 실행 상태를 인스턴스에 보관하지 않도록 생성을 막는다.
     */
    private ConcurrentScenario() {
    }

    /**
     * 두 작업을 barrier에서 함께 출발시켜 결과를 입력 순서로 반환한다. 각 Future 대기와 executor 종료 대기에 각각 timeout을 적용하므로 전체 경과 시간 상한과는 다르다.
     */
    ...
    /**
     * barrier 통과 뒤 작업의 반환값 또는 Throwable을 결과에 담는다. barrier 대기 자체의 실패는 Future 예외로 전파한다.
     */
    ...
    record Result<T>(T value, Throwable failure) {
        /**
         * 정상 반환값과 null 실패값으로 작업 결과를 구성한다.
         */
        ...
        /**
         * 예외와 null 반환값을 저장해 다른 작업의 결과와 함께 비교할 수 있게 한다.
         */
        ...
        /**
         * 반환값이 null인지와 무관하게 실패 예외가 없으면 성공으로 판정한다.
         */
        boolean succeeded() {
            return failure == null;
        }
