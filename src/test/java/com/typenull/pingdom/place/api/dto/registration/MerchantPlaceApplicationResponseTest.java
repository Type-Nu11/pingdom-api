...
class MerchantPlaceApplicationResponseTest {
    ...
    /** 사업자 신규 신청 응답이 유형·장소명·태그와 월요일 영업시간을 보존하는지 확인한다. */
    @Test
    ...
    /** 기존 장소 운영권 신청은 신규 장소 입력 응답을 null로 반환하는지 확인한다. */
    @Test
    ...
    /** 필수 첨부를 갖춘 신청을 제출·승인·완료하면 응답에 COMPLETED와 연결 장소 ID가 반영되는지 확인한다. */
    @Test
    ...
    /** 고정 장소 정보와 영어 메뉴 태그를 가진 사업자 신규 신청 초안을 만든다. */
    ...
    /** 영업일 JSON 직렬화·역직렬화에 필요한 JavaTimeModule을 등록한다. */
    private ObjectMapper objectMapper() {
        return new ObjectMapper().registerModule(new JavaTimeModule());
    }

    /** 문서 유형마다 다른 해시를 가진 필수 첨부를 만들어 제출 조건을 충족시킨다. 파일 저장소는 호출하지 않는다. */
