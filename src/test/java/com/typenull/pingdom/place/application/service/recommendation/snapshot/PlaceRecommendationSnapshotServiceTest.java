...
class PlaceRecommendationSnapshotServiceTest {
    ...
    /**
     * 전체 장소 집계 갱신의 잠금·재조회 호출을 모의로 관찰할 서비스를 준비합니다.
     */
    @BeforeEach
    void setUp() {
    ...
    /**
     * 최초 조회에는 없고 장소 잠금 뒤 발견된 스냅샷을 재사용해 노출을 3에서 4로 늘리고 콘텐츠 집계를 새로 조회하지 않는지 확인합니다.
     */
    @Test
    ...
    /**
     * 중복·역순 장소 입력을 정렬한 고유 ID 일괄 잠금으로 처리하고 개별 장소 잠금은 호출하지 않는지 확인합니다.
     */
    @Test
    ...
    /**
     * 누락 스냅샷의 초기 사진 수를 제공할 장소 fixture를 만듭니다.
     */
