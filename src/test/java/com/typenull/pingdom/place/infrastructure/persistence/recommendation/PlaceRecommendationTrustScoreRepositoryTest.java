...
class PlaceRecommendationTrustScoreRepositoryTest {
    ...
    /** PostGIS·삼중문자 확장을 준비하고 실제 Flyway 마이그레이션으로 쿼리 대상 schema를 만든다. */
    @BeforeAll
    ...
    /** PostgreSQL에 중복 승인 제보·정책 없는 제보자·반려 제보를 넣고 실제 신뢰 점수 SQL이 장소 하나에 0.54를 반환하는지 확인한다. */
    @Test
    ...
    /** 컨테이너의 JDBC 연결을 새로 연다. 호출 측 try-with-resources에서 닫는다. */
    private static Connection connection() throws Exception {
        return DriverManager.getConnection(postgres.getJdbcUrl(), postgres.getUsername(), postgres.getPassword());
    }

    /** 저신뢰·신규·반려 제보자와 검토 관리자, 공통 장소를 실제 DB에 삽입한다. */
    ...
    /** 유형·제보자가 지정된 승인 제보를 넣어 동일 제보자 중복 집계 여부를 검사한다. */
    ...
    /** 승인 집계에서 제외되어야 하는 별도 제보자의 반려 행을 삽입한다. */
