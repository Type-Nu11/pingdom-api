...
/**
 * 혜택의 공개 상태, 발급 수량 및 쿠폰 만료 정책을 보유합니다.
 * 무제한 정책의 totalQuantity는 null이며, 발급 기간은 시작 포함·종료 제외입니다. 동시 발급 잠금은 저장소 호출자가 담당합니다.
 */
@Entity
@Getter
...
public class TouristOffer {
    ...
    @Enumerated(EnumType.STRING)
    @Column(name = "eligibility_policy", nullable = false, length = 30)
    private CouponEligibilityPolicy eligibilityPolicy;

    @Enumerated(EnumType.STRING)
    @Column(name = "inventory_policy", nullable = false, length = 20)
    private CouponInventoryPolicy inventoryPolicy;

    @Enumerated(EnumType.STRING)
    @Column(name = "expiry_policy", nullable = false, length = 50)
    private CouponExpiryPolicy expiryPolicy;
    ...
    private TouristOffer(
        ...
        validateQuantity(totalQuantity, couponValidityDays, inventoryPolicy);
    ...
    public static TouristOffer draft(
        ...
        return new TouristOffer(
                merchantOwnerUserId, placeId, title, description, benefitDescription,
                startsAt, endsAt, totalQuantity, couponValidityDays,
                eligibilityPolicy, inventoryPolicy, expiryPolicy, now
        );
    }
    ...
    /**
     * 공개 기간과 제한 재고를 검사한 뒤 발급 수량을 하나 늘리고 정책별 쿠폰 만료 시각을 반환합니다.
     * 발급 불가 상태와 재고 소진은 서로 다른 예외로 구분하며, 실제 쿠폰 생성·저장은 수행하지 않습니다.
     */
    public LocalDateTime issueCoupon(LocalDateTime now) {
        ...
        if (inventoryPolicy == CouponInventoryPolicy.LIMITED
        ...
        return switch (expiryPolicy) {
            case ISSUE_PLUS_DAYS -> validityEnd;
            case OFFER_END -> endsAt;
            case ISSUE_PLUS_DAYS_CAPPED_BY_OFFER_END ->
                    validityEnd.isBefore(endsAt) ? validityEnd : endsAt;
        };
    }
    ...
    public boolean isSoldOut() {
        return inventoryPolicy == CouponInventoryPolicy.LIMITED
                && issuedQuantity >= totalQuantity;
    }
    ...
    private static void validateQuantity(
        ...
        boolean validQuantity = inventoryPolicy == CouponInventoryPolicy.UNLIMITED
                ? totalQuantity == null
                : totalQuantity != null && totalQuantity > 0;
        if (!validQuantity || couponValidityDays < 1 || couponValidityDays > 365) {
