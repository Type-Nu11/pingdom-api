...
/**
 * 관리자 장소 검색에 분류 alias와 숫자 ID 검색을 적용하고 관광 카테고리를 배치 조합.
 * 상세의 게시글은 숨김 상태를 포함해 최대 20개를 반환하고, postCount는 해당 제목 검색에 맞는 전체 건수.
 * 성장 집계와 숨김 사진 수는 게시글 검색어와 별도로 조회.
 */
@Service
...
public class AdminMapPlaceLookupQueryService {
    ...
    /**
     * 장소명·주소 검색어와 숫자로 해석 가능한 ID, 정규화한 카테고리 별칭으로 검색하고 관광 카테고리를 배치로 조합.
     * page는 1 이상·limit는 1~100으로 보정하고 정렬이 없으면 최신 ID순을 사용.
     */
    ...
    /**
     * 존재하는 장소에 제목 검색과 정렬을 적용한 게시글 최대 20개, 검색에 맞는 전체 게시글 수, 장소 성장 정보를 결합.
     * 게시글에는 숨김 상태도 포함하며 숨김 사진 수는 제목 검색과 별도로 집계. 장소가 없으면 PLACE_NOT_FOUND.
     */
    @Transactional(readOnly = true)
