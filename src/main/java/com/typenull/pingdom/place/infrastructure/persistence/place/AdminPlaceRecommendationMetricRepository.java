...
/**
 * 관리자 지표 목록을 전체/버전 스냅샷 또는 기간 내 원본 이벤트 집계로 정렬.
 * 장소명 LIKE 필터 외에 운영·공개 상태 제한은 없으며 데이터가 없는 장소도 LEFT JOIN으로 포함.
 * 노출 0인 CTR·전환율은 0으로 두고 지표별 동률 조건 후 장소 ID 오름차순으로 정렬.
 */
public interface AdminPlaceRecommendationMetricRepository extends Repository<MapPlace, Long> {
    ...
    /**
     * cutoff 시각을 포함한 이후의 노출·클릭을 집계하며 상한 시각은 미지정.
     * recommendationVersion이 빈 문자열이면 버전 전체를 포함하고 장소명 필터 범위로 합계를 제한.
     */
    @Query(value = """
