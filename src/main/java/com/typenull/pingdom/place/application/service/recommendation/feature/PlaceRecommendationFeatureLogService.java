...
/**
 * 실제로 표시한 후보의 거리·신호별 점수·최종 순위를 저장하고 클릭 전환과 연결할 로그 ID를 찾음.
 * 연결할 requestId나 로그가 없으면 null을 반환하므로 특성 로그 부재 시에도 전환 기록 가능.
 */
@Service
...
public class PlaceRecommendationFeatureLogService {
    ...
    /**
     * 실제로 표시한 후보의 순위·신호별 점수를 요청·사용자·정책 버전과 함께 일괄 저장.
     * 빈 목록은 건너뛰며 null 목록은 허용 불가. 저장 실패는 전파되어 참여 중인 추천 요청 트랜잭션도 실패할 수 있음.
     */
    @Transactional
    ...
    /**
     * 요청·사용자·장소·버전이 일치하는 특성 로그 중 가장 작은 ID를 전환 귀속 대상으로 반환.
     * 요청 ID가 공백이거나 일치 로그가 없으면 null을 반환하여 특성 로그 없는 전환도 허용.
     */
