...
/**
 * 기본 점수 또는 실험군의 출처별 할당량으로 후보 포트폴리오를 만든 뒤 MMR로 유사 장소의 중복 노출을 완화.
 * 출처별 후보는 장소 ID로 중복 제거하고 반올림한 할당량을 순서대로 채운 뒤 부족분을 기본 점수로 보충.
 */
@Service
...
class PlaceRecommendationPortfolioService {
    ...
    /**
     * 첫 후보는 최종 점수로 선택하고 이후에는 이미 선택한 장소와의 최대 유사도를 감점.
     * 정책의 relevance 가중치가 1이면 유사도 감점을 없애고 원래 관련도 점수만 사용.
     */
