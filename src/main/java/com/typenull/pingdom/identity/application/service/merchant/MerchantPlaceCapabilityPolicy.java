...
/**
 * 탈퇴·현재 정지 여부와 장소 팀 역할을 바탕으로 기능별 접근을 제한합니다.
 * 팀원 행이 없는 경우에만 기존 소유 매핑을 OWNER로 인정하므로 비활성 팀원은 소유 매핑으로 우회하지 않습니다.
 */
@Component
...
public class MerchantPlaceCapabilityPolicy {
    ...
    /**
     * 탈퇴·정지되지 않은 요청자의 활성 장소 팀 역할로 지정 기능 접근을 검사하고 미충족 시 팀 권한 오류를 던집니다.
     * 팀원 행이 없을 때만 소유 매핑을 OWNER로 인정하며, 비활성 팀원은 소유 매핑으로 우회할 수 없습니다.
     */
