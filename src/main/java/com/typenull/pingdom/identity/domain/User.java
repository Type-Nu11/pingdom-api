...
/**
 * 회원 식별 정보, 계정 상태와 현재 refresh 세션을 보관하는 회원 엔티티입니다.
 * 비밀번호·프로필 변경과 정지·탈퇴 상태 전이를 수행하며 관련 데이터 정리나 외부 저장소 처리는 서비스에 맡깁니다.
 */
@Getter
@Entity
@Table(name = "users")
...
public class User {
    ...
    /**
     * 만료 시각이 없거나 현재 시각이 그 이후일 때 만료로 판정합니다.
     * 재설정 토큰과 달리 만료 시각과 정확히 같은 시점은 이 비교에서 만료가 아닙니다.
     */
    ...
    /**
     * 새 세션은 SHA-256 해시와 비교하고 기존 평문 저장값도 호환을 위해 허용합니다.
     * 평문 호환은 회전 시 해시로 교체되며 JWT 서명·만료 유효성은 이 메서드가 검사하지 않습니다.
     */
