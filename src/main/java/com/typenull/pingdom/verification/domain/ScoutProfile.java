...
/**
 * Scout 프로필의 심사 상태와 표시 정보, 마지막 심사 이력을 관리한다.
 * 프로필 상태 전이만 담당하며 활동 자격 기간은 ScoutActivityEligibility에서 별도로 판단한다.
 * version은 JPA 낙관적 잠금에 사용된다.
 */
@Entity
@Getter
...
public class ScoutProfile {
    ...
    /**
     * 양수 사용자 ID와 비어 있지 않은 표시 이름으로 심사 대기 프로필을 생성한다.
     * 이름·소개는 앞뒤 공백을 제거하고 빈 소개는 null로 보관한다.
     */
    ...
    /** 대기 또는 활성 상태에서만 표시 정보를 수정한다. 입력과 시각을 먼저 검증한 뒤 필드를 변경한다. */
    ...
    /** 대기 또는 정지 프로필을 활성화하고 심사자·시각을 갱신한다. 이전 정지 사유는 지운다. */
    ...
    /** 활성 프로필에 한해 필수 사유를 기록하고 정지한다. 유효하지 않은 사유·심사자·시각은 변경 전에 거부한다. */
    ...
    /** 활성 또는 정지 프로필만 사유와 함께 회수한다. 대기 및 이미 회수된 상태에서는 허용하지 않는다. */
    ...
    /** 마지막 심사자·시각·사유를 기록하고 수정 시각을 함께 갱신한다. */
    ...
    /** 필수 식별자가 양수인지 확인하며 실패 메시지에 인자 이름을 포함한다. */
    ...
    /** 앞뒤 공백을 제거한 문자열을 반환하고 null 또는 공백뿐인 필수 값은 거부한다. */
    ...
    /** 선택 문자열의 공백을 정리하고 null 또는 빈 값을 null로 통일한다. */
    ...
    /** 상태 변경 이력을 남길 시각이 반드시 제공되도록 null을 거부한다. */
