...
/** 서버 수신 시각과 연속 관측 간격으로 장소 체류 시간을 판정. */
@Service
...
public class VisitVerificationService {
    ...
    /**
     * 관광객 계정을 확인하고 지정 장소의 체류 인증을 시작하거나 당일 기존 세션을 반환.
     * 새 세션에만 현재 정책을 확정하므로 설정이 바뀌어도 기존 세션의 반경·체류 시간은 유지됨.
     */
    @Transactional
    ...
    /** 장소 ID를 신뢰하지 않고 좌표 주변의 단일 공개 장소를 서버가 선택. */
    @Transactional
    public VisitVerificationSessionResponse startForeground(Long userId,
        ...
        // 진행 중인 동일 장소 세션은 새 후보 탐색보다 먼저 복구.
    ...
    /**
     * 후보 간 거리 차이가 GPS 정확도 두 배보다 작거나 같으면 실제로 어느 장소가 더 가까운지 보장할 수 없음.
     * Repository 정렬(distance, placeId)로 재현성은 확보하되, 불확실한 동률의 임의 선택은 배제.
     */
    ...
    /**
     * 당일 진행 세션을 최근 서버 확인 시각 순으로 검사해 재사용 가능한 첫 세션을 반환.
     * 재사용 검사 과정에서 오래된 세션을 만료 상태로 바꿀 수 있음.
     */
    ...
    /**
     * TTL 또는 관측 공백을 넘긴 세션은 만료 처리.
     * 그 밖에는 현재 공개·운영 장소의 저장된 반경 안에 있을 때만 재사용.
     */
    ...
    /**
     * 같은 사용자·장소·서울 날짜의 완료 세션을 우선하고 이어 진행 세션을 찾음.
     * 기존 세션 반환 경로에서는 관측 정확도·시각만 검증하며 장소 상태·거리·세션 만료는 재판정 대상에서 제외.
     * 새 세션이면 장소 및 반경을 확인하고 활동 세션 유일 제약 위반을 별도 중복 오류로 변환.
     */
    ...
    /**
     * 소유자 조건과 쓰기 잠금으로 세션을 조회해 동시 관측 처리를 직렬화.
     * 종료 상태는 그대로 반환하고 TTL·공백 초과는 만료, 반경 이탈은 이탈 상태로 기록.
     * 충분한 서버 경과 시간이 쌓이면 같은 트랜잭션에서 체류 체크인을 저장하고 세션을 완료.
     */
    @Transactional
    public VisitVerificationSessionResponse submitObservation(Long userId, Long sessionId,
        ...
        // 클라이언트 관측 시각은 순서·유효성 검증에 사용하고 체류 누적은 서버 시각을 기준으로 함.
    ...
    /**
     * 본인 세션 조회 시에도 쓰기 잠금을 잡고 만료 조건이면 상태를 갱신.
     * 조회 API의 만료 처리를 위해 쓰기 가능한 트랜잭션 적용.
     */
    @Transactional
    ...
    /**
     * 완료 시점의 서울 날짜로 DWELL_VERIFIED 체크인을 저장.
     * 당일 체류 인증 유일 제약 위반만 전용 중복 오류로 변환하며 다른 DB 실패는 그대로 전달.
     */
    ...
    /** 운영 중이며 공개된 장소만 허용하고, 미존재·비공개·비운영을 같은 장소 부재 오류로 처리. */
    ...
    /**
     * 정확도는 전역 상한과 세션 반경 중 작은 값 이하만 허용.
     * 관측 시각은 과거 TTL부터 미래 허용 오차까지 경계를 포함해 인정.
     */
    ...
    /** USER 역할의 미탈퇴·현재 미정지 계정만 체류 인증에 참여할 수 있음. */
    ...
    /** 원인 예외 체인에서 제약 이름을 찾아 알려진 중복 오류만 변환하도록 구분. */
