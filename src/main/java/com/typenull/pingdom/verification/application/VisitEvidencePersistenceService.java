...
/**
 * 체크인 소유권 확인과 증빙 메타데이터의 DB 저장·조회를 담당.
 * 업로드 조율 서비스에서 별도 Bean으로 호출하여 DB 트랜잭션을 적용하며 S3 작업은 담당 범위 외.
 */
@Service
public class VisitEvidencePersistenceService {
    ...
    /**
     * 저장 트랜잭션 안에서 소유권을 다시 확인하고 증빙을 즉시 flush함.
     * 지정한 체크인 유일 제약 위반만 이미 등록된 증빙 오류로 변환하며 다른 무결성 오류는 유지.
     * 저장 실패는 호출한 업로드 서비스로 전달되어 S3 객체 정리의 계기가 됨.
     */
    @Transactional
    ...
    /** 본인 체크인 존재 여부를 먼저 확인한 뒤 체크인과 사용자 두 조건으로 증빙을 조회. */
    ...
    /**
     * 체크인 ID와 사용자 ID가 함께 일치해야 하며, 없거나 타인 소유이면 동일한 부재 오류를 냄.
     * 같은 Bean의 save/getOwned에서 호출하면 새 트랜잭션을 열지 않고 호출 메서드의 범위에서 실행됨.
     */
    @Transactional(readOnly = true)
    ...
    /** 원인 예외 체인에서 Hibernate가 보고한 제약 이름을 대소문자 구분 없이 비교. */
