...
/**
 * 업로드된 JPEG·PNG의 실제 바이트와 선언 타입을 대조하고 이미지 크기를 제한.
 * 픽셀 데이터를 다시 인코딩한 결과만 저장 계층에 전달하며 원본 파일명을 저장용 메타데이터로 정규화.
 */
@Component
public class VisitEvidenceFileValidator {
    ...
    /**
     * 빈 파일, 용량 초과, 형식 불일치 또는 판독 불가 이미지를 도메인 오류로 거부.
     * 요청 크기·실제 바이트 길이·재인코딩 결과 크기를 각각 확인하여 모든 단계에 용량 제한 적용.
     */
    ...
    /**
     * 첫 번째 이미지의 너비·높이·픽셀 수를 먼저 확인한 뒤 디코딩하고 다시 인코딩.
     * 가로/세로 8,000 및 총 3,600만 픽셀을 초과하면 거부하고 JPEG는 RGB로 정규화.
     * 원본 스트림의 메타데이터를 그대로 복사하지 않으며 출력 인코더가 없으면 형식 오류로 처리.
     */
    private byte[] decodeAndReencode(byte[] bytes, ImageType type) throws IOException {
        ...
        // JPEG 출력에 맞지 않는 색상 모델은 흰 배경의 RGB 이미지에 그려 변환.
    ...
    /** MIME 타입의 앞뒤 공백과 대소문자를 정규화하며, 누락된 값은 빈 문자열로 비교. */
    ...
    /**
     * 양식에 포함된 경로 부분을 제거하고 빈 이름에는 evidence 확장자 이름을 사용.
     * 255자를 넘으면 뒤쪽 255자만 유지. 실제 S3 객체명과 별개의 원본 파일명 메타데이터.
     */
    ...
    private enum ImageType {
        ...
        /** JPEG·PNG의 시작 시그니처를 구분. 이미지 본문의 유효성은 후속 디코딩에서 확인. */
    ...
    /** 용량·형식 검사를 통과해 재인코딩한 바이트와 정규화된 파일 메타데이터. */
