...
/**
 * 분석 기준일과 요청 조건을 JSON으로 직렬화하고 응답 구조·근거 제시 지침을 프롬프트에 결합합니다.
 * 모델에 전달하는 지침 자체가 데이터의 진실성이나 도구 호출을 보장하지 않으므로 응답 검증은 별도로 수행합니다.
 */
@Service
...
public class LocationAnalysisPromptFactory {
    ...
    private static final String DESIGN_REFERENCE = """
            ...
            - 섹션 구조: 01 표지·종합 입지 평가, 02 상권 개요·추천 장소,
              ...
              06 사업성·실행 전략, 07 데이터 신뢰도·출처·제한사항
    ...
    /**
     * 요청 조건을 JSON으로 직렬화해 기준일·고정 응답 지침과 결합한 프롬프트를 반환한다.
     * 직렬화 실패는 AI_RESPONSE_INVALID로 변환하며 이 단계에서는 모델이나 MCP 도구를 호출하지 않는다.
     */
    ...
    private String buildPrompt(String criteria, LocalDate analysisBasisDate) {
        return """
                ...
                서버는 7개 고정 섹션으로 보고서를 구성하므로 각 섹션의 summary와 수치를 생략하지 않는다.
                ...
                상권·추천 장소, 타깃 고객, 유동인구·영업시간, 경쟁 환경·주변 시설,
                사업성·실행 전략, 데이터 신뢰도·출처의 7개 페이지 구간이 고정된다. 데이터가 없는 경우에도
