...
/** 처리할 이벤트 선점과 오래된 PROCESSING 복구를 각각 짧은 DB 트랜잭션으로 수행한다. */
@Service
...
public class OutboxEventClaimService {
    ...
    /**
     * 현재 시각까지 실행 기한이 된 PENDING/RETRY를 배치 크기만큼 잠금 조회하고 PROCESSING으로 바꾼다.
     * 실제 handler 실행은 선점 트랜잭션 밖의 worker가 담당한다.
     */
    @Transactional
    ...
    /**
     * processingTimeout보다 오래 진행 중인 이벤트를 잠금 조회해 실패 횟수와 backoff를 반영한다.
     * 실패 한도에 도달하면 FAILED가 될 수 있으며 복구 조회 건수를 반환한다.
     */
    @Transactional
