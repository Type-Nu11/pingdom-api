...
/**
 * 중복 키와 JSON payload를 갖는 Outbox 행을 저장한다.
 * 자체 트랜잭션 선언이 없으므로 업무 변경과 원자적으로 저장하려면 호출자가 트랜잭션을 제공해야 한다.
 */
@Component
...
public class OutboxEventPublisher {
    ...
    /**
     * 같은 중복 키가 이미 있으면 null, 새로 저장하면 이벤트 ID를 반환한다.
     * 존재 조회만으로 동시 발행 경쟁을 막지는 못하며 DB 유일 제약과 호출자의 잠금 규칙이 별도로 필요하다.
     */
    ...
    /** payload를 JSON으로 직렬화하며 실패를 IllegalStateException으로 전달해 발행을 중단한다. */
