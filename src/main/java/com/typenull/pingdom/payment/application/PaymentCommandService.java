...
/**
 * 결제 준비·완료 트랜잭션 사이에서 외부 결제 사업자를 호출.
 * 사업자 호출과 DB 반영은 하나의 원자적 작업이 아니며, 응답 불명은 실패로 확정하지 않고 처리 중 상태를 남김.
 */
@Service
...
public class PaymentCommandService {
    ...
    /**
     * 같은 멱등 키의 완료 거래는 재사용하고, PROCESSING 거래는 같은 키로 사업자 승인 호출을 다시 수행.
     * 중복 승인의 방지는 PaymentProvider의 멱등 계약에 의존하며, 확정 거절만 FAILED로 기록.
     */
    ...
    /**
     * 환불 처리 중 상태를 먼저 저장하고 거래 ID 기반 키로 전액 환불을 요청.
     * 확정 거절이면 PAID로 되돌리지만, 결과 불명 또는 외부 성공 후 DB 실패의 자동 복구는 처리 범위 외.
     */
