...
Pingdom Backend는 이벤트 기반 모듈러 모놀리스를 기본 구조로 사용한다.

CQRS는 전체 기능에 일괄 적용하지 않는다.

대신 조회 복잡도가 높고 응답 조합이 많은 영역에 선택적으로 적용한다.

이 구조는 현재 Pingdom의 기능 복잡도와 운영 현실을 동시에 만족시키는 실무형 구조로 간주한다.
